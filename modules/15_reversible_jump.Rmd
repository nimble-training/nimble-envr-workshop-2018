...
Here's the BUGS code that implements the model specified earlier, modified slightly to allow for variable selection by adding an indicator that will "turn on" or "turn off" the Avandia effect.
...
To go from the reduced model, $M^r$, to full model, $M^f$, we need to propose an auxiliary variable, $u$, that augments the dimension of the model and a transformation to produce the new coefficient, $\theta^*$, given the auxiliary variable. The simplest thing to do is:
...
For those of you familiar with RJ, the lack of Jacobian here occurs because the transformation $\theta^*=u$ has Jacobian equal to 1.
...
```{r, setup-rj, eval=FALSE}
 setup = function( model, mvSaved, target, control ) {
   # target is equal to 'indic' in our example model
...
slice_var_sel_wrapper <- nimbleFunction(
    contains = sampler_BASE,
    setup = function(model, mvSaved, target, control) {
        # We create a slice sampler by calling sampler_slice generator to
        # instantiating a slice sampler nimbleFunction.
        # 'regular_slice_sampler' will be compiled as part of overall compile.
    ...
    run = function() {
        # Note we can call $run method of another nimbleFunction.
...
modelParamSel <- nimbleModel(code = codeParamSel, data = data, inits = inits,
                      constants = constants)
cModelParamSel = compileNimble(modelParamSel)

conf <- configureMCMC(modelParamSel, thin = 10, monitors = c('theta', 'indic'))
conf$removeSamplers(c('theta', 'indic'))
## special changes for variable selection:
conf$addSampler('indic', 'RJ_var_sel',
                         control = list(coef = 'theta', scale = 0.5))
conf$addSampler('theta', 'slice_var_sel_wrapper',
                         control = list(indicator = 'indic'))

mcmc <- buildMCMC(conf)
cmcmc <- compileNimble(mcmc, project = modelParamSel)
samples <- runMCMC(cmcmc, niter = 11000, nburnin = 1000)
```
...
par(mfrow = c(1,2))
ts.plot(samples[ , 'indic'], xlab = 'iteration', ylab = 'theta indicator',
                    main = 'theta indicator')
ts.plot(samples[ , 'theta'], xlab = 'iterations', ylab = 'theta',
...
That's less strong evidence for an Avandia effect than I interpreted based on the posterior from module 9...
