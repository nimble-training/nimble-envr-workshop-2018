...
**ENVR workshop attendees: please verify that you can run the code on this slide. If you'd like, feel free to look through and run the code in the remainder of this module that would be helpful.**

```{r, litters-nimbleMCMC, fig.caption = '', fig.width=16}
...
samples <- nimbleMCMC(code = littersCode, data = littersData, inits = littersInits,
                      ...
                      thin = 1, niter = 1100, nburnin = 100, nchains = 1, setSeed = TRUE)
par(mfrow = c(1, 4))
...
In order to talk about MCMC customization in the next module, we first need to see the 'manual' steps of running an MCMC in NIMBLE (as a contrast to the on-click MCMC seen in the previous slide).
...
Note that `nimbleMCMC()` combined steps 1-4 (and in fact does not even require you to create the model).
...
- To compare multiple MCMC configurations, use `compareMCMCs()`.
