---
title: "Bayesian nonparametrics example: Meta analysis"
...
The example is of the side effects of a very popular drug for diabetes called Avandia. The question is whether Avandia use increases the risk of myocardial infarction (heart attack). There are 48 studies (the 49th study in the data file is different in some ways and excluded here), each with treatment and control arms.
...
samples <- nimbleMCMC(code = codeParam, data = data, inits = inits,
                      constants = constants,
                      thin = 10, niter = 21000, nburnin = 1000,
                      monitors = c("mu", "tau", "theta", "gamma"),
...
samplesBNP <- nimbleMCMC(code = codeBNP, data = data, inits = inits,
               constants = constants,
               monitors = c("theta", "gamma", "alpha", "xi", "mu0",
                        "sd0", "a0", "b0"),
               thin = 10, niter = 21000, nburnin = 1000, nchains = 1,
               setSeed = TRUE)

gammaCols <- grep('gamma', colnames(samplesBNP))
xiCols <- grep('xi', colnames(samplesBNP))

par(mfrow = c(2, 3), cex = 1, fig.width=12, fig.height=12)
ts.plot(samplesBNP[ , 'theta'], xlab = 'iteration', ylab = expression(theta),
                    main = 'mixing for theta')
...
# NIMBLE's samplers for BNP models
